...
export class SystemFlowCard extends LitElement {
  ...
  private displayValue = (value, abs: boolean = false, unit: string | null = null): number | string | undefined => {
    let newValue: string | number = value;
    if(abs) {
      newValue = round(Math.abs(coerceNumber(newValue)), 2)
    }
    if(unit) {
  ...
  private getEntityValue = (ent: HassEntity | string | undefined, abs: boolean = false, showUnit: boolean = false): number | string | undefined => {
    const entity = typeof ent === 'string' ? this.getEntity(ent) : ent;
    if(!entity)
      return undefined
    ...
    return this.displayValue(value, abs, unit)
  }
  ...
  private elementValueArrows = (element: CalculatedElementDef) => 
    element.calculations.systemTotal
      ? html`
          <span>
            ...
            ${this.displayValue(element.calculations.systemTotal, element.position !== 'middle', this.getElementUnit(element))}
  ...
  private elementToHtml = (element: CalculatedElementDef) => html`
    <div class="circle-container" style="opacity: ${!element.calculations.systemTotal && this._config?.fadeIdylElements ? .25 : 1};">
      <div class="circle" style="border-color: ${this.getElementColor(element)}">
        ${element.position === 'bottom'
          ? this.elementValueArrows(element)
          : html`<span>${element.extra ? this.getEntityValue(element.extra, false, true) : ' '}</span>`
        }
        <span>
          <ha-icon style="fill: ${this.getElementColor(element)}" icon="${this.getElementIconId(element)}"></ha-icon>
        </span>
        ${element.position === 'bottom'
          ? html`<span>${element.extra ? this.getEntityValue(element.extra, false, true) : ' '}</span>`
          : this.elementValueArrows(element)
  ...
  protected render(): TemplateResult {
    ...
    const elements = configElements.map((element) : CalculatedElementDef => {
      ...
      toSystem = typeof element.value === "string"
        ? coerceNumber(this.getEntityValue(element.value))
    ...
    const posDimensionMap = {
      "left": "position: absolute; left: 0; top: 0; width: calc(50% - 42px); height: 100%;",
      "top": "position: absolute; left: 0; top: 0; width: 100%; height: calc(50% - 42px);",
      "right": "position: absolute; left: calc(50% + 42px); top: 0; width: calc(50% - 42px); height: 100%;",
      "bottom": "position: absolute; left: 0; top: calc(50% + 42px); width: 100%; height: calc(50% - 42px);",
    }
        ...
        <div class="card-content">
          <div class="row">
            ...
            <div class="col">
              ...
              <div class="row">
                ${elementPositions.middle.map(element => this.elementToHtml(element))}
              </div>
